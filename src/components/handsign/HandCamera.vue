  ...
  <section class="border-[3px] border-brand-teal bg-white p-4">
    ...
    <div class="relative mx-auto aspect-[4/3] w-full max-w-[760px] overflow-hidden border-[3px] border-brand-teal bg-gray-100">
      ...
      <div v-if="!isRunning" class="absolute inset-0 grid place-items-center bg-white/90 p-6 text-center">
        <button class="border-[3px] border-brand-teal bg-brand-amber px-5 py-3 text-xs font-black" type="button" @click="$emit('start')">
          Start Camera
        </button>
      </div>
    ...
    <p v-if="error" class="mt-3 border-[3px] border-brand-teal bg-brand-rose/10 px-3 py-2 text-xs font-black text-red-700" role="alert">{{ error }}</p>
