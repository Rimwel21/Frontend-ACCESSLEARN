<template>
  <div class="min-h-screen bg-white">
    ...
    <div class="grid gap-5 px-7 py-6 xl:grid-cols-[minmax(0,1fr)_365px]">
      <div class="space-y-5">
        <SignLanguageToggle v-if="activeActivity && typedAnswersAllowed" v-model="signLanguageMode" :disabled="isActivityCompleted" />
          ...
          <div v-if="activeQuestionParsed.choices.length" class="mt-4 grid gap-3 sm:grid-cols-2 2xl:grid-cols-4">
            <button
              ...
              :class="[
                ...
                isActivityCompleted || !typedAnswersAllowed ? 'cursor-not-allowed opacity-60' : ''
              ]"
        ...
        <div v-if="activeActivity" class="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(320px,0.55fr)]">
          <HandCamera
            v-if="forcedSignLanguageMode && !isActivityCompleted"
          ...
          <PredictionDisplay
            ...
            :sign-mode="forcedSignLanguageMode"
...
function selectChoice(choice: { letter: string; text: string }) {
  if (isActivityCompleted.value || !typedAnswersAllowed.value) return
...
const defaultModeLabel = computed(() => {
  if (!typedAnswersAllowed.value) return 'Sign Language Mode only'
  return isHearingImpaired.value ? 'Sign Language Mode on' : 'Text input'
})
const currentAnswer = computed(() => forcedSignLanguageMode.value ? answer.value : textAnswer.value)
...
watch(forcedSignLanguageMode, async (enabled) => {
...
watch(answer, value => {
  if (!forcedSignLanguageMode.value || isActivityCompleted.value) return
...
watch(typedAnswersAllowed, allowed => {
  if (!allowed) {
    signLanguageMode.value = true
    textAnswer.value = ''
  }
})
...
onMounted(async () => {
  ...
  if (!defaultWasApplied.value) {
    signLanguageMode.value = !typedAnswersAllowed.value || isHearingImpaired.value
...
async function submitAnswer() {
  ...
  const submittedMode = forcedSignLanguageMode.value ? 'Sign Language Mode' : 'Text Mode'
