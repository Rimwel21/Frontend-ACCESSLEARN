<template>
  <div class="flex flex-col lg:flex-row h-screen bg-white font-body overflow-hidden w-full">
    ...
    <header class="lg:hidden flex items-center justify-between p-4 border-b border-gray-100 bg-white z-50">
      ...
      <button @click="isMobileMenuOpen = !isMobileMenuOpen" class="text-gray-500 hover:text-ink">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
      </button>
    </header>

    <!-- Mobile Menu Overlay -->
    <div v-if="isMobileMenuOpen" class="fixed inset-0 bg-white z-40 pt-20 px-6 lg:hidden flex flex-col space-y-4">
       ...
       <RouterLink to="/register" @click="isMobileMenuOpen=false" class="flex items-center gap-3 px-4 py-3 rounded-xl text-ink-soft font-bold">Register</RouterLink>
       <RouterLink to="/login" @click="isMobileMenuOpen=false" class="flex items-center gap-3 px-4 py-3 rounded-xl text-ink-soft font-bold">Login</RouterLink>
    </div>
    ...
    <aside class="hidden lg:flex w-[260px] bg-white border-r border-gray-100 flex-col justify-between shrink-0 relative z-20 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
      <div>
        ...
        <nav class="px-4 space-y-2">
          <a href="#" class="flex items-center gap-3 px-4 py-3 bg-gray-100 rounded-xl text-ink font-bold hover:bg-gray-200 transition-colors">
          ...
          <RouterLink to="/register" class="flex items-center gap-3 px-4 py-3 rounded-xl text-ink-soft font-bold hover:bg-gray-50 hover:text-ink transition-colors">
          ...
          <RouterLink to="/login" class="flex items-center gap-3 px-4 py-3 rounded-xl text-ink-soft font-bold hover:bg-gray-50 hover:text-ink transition-colors">
    ...
    <main class="flex-1 flex flex-col relative grid-bg h-full lg:overflow-y-auto overflow-x-hidden">
      ...
      <header class="hidden lg:flex h-20 items-center justify-between px-8 shrink-0">
        ...
        <div class="flex items-center gap-4 text-gray-400 px-4">
          <div class="w-9 h-9 rounded-full bg-gray-200 border-2 border-white shadow flex items-center justify-center overflow-hidden cursor-not-allowed">
      ...
      <div class="p-6 lg:px-8 lg:pb-12 w-full max-w-[1300px] mx-auto space-y-6 flex-1 flex flex-col justify-center relative">
        ...
        <!-- Hero Banner -->
        <div class="relative w-full rounded-[24px] bg-gradient-to-r from-brand-blue to-brand-violet shadow-xl overflow-hidden min-h-[300px] flex flex-col justify-center">
          ...
          <div class="relative z-20 text-white w-full px-8 py-10 lg:pl-[48%] lg:pr-12">
            ...
            <p class="text-white/95 text-[13px] sm:text-sm mb-8 max-w-[500px] font-medium leading-relaxed">
            ...
            <div class="flex flex-wrap items-center gap-4">
              <RouterLink to="/register" class="bg-[#00E676] hover:bg-[#00c853] text-white px-8 py-3 rounded-full font-bold text-sm shadow-lg transition-transform hover:scale-105 text-center flex-1 sm:flex-none">Get Started</RouterLink>
        ...
        <div id="features" class="grid grid-cols-2 lg:grid-cols-5 gap-3 lg:gap-4 relative z-20 xl:-mt-10">
          ...
          <div class="bg-white rounded-2xl p-4 lg:p-5 flex flex-col items-center text-center shadow-[0_4px_20px_rgba(0,0,0,0.04)] border border-gray-100/50 hover:-translate-y-1 transition-transform group">
            ...
            <h3 class="font-bold text-ink text-[13px] lg:text-sm mb-2 leading-tight">TTS and Audio</h3>
            <p class="text-[10px] lg:text-[11px] text-ink-soft leading-relaxed font-medium">For visual support learners.<br/> Listen to lesson summaries and narration before each quiz.</p>
          </div>
          <!-- Card 2 -->
          <div class="bg-white rounded-2xl p-4 lg:p-5 flex flex-col items-center text-center shadow-[0_4px_20px_rgba(0,0,0,0.04)] border border-gray-100/50 hover:-translate-y-1 transition-transform group">
            <div class="text-3xl lg:text-4xl mb-3 h-12 flex items-center justify-center group-hover:scale-110 transition-transform">🧏💬</div>
            <h3 class="font-bold text-ink text-[13px] lg:text-sm mb-2 leading-tight">Sign and Captions</h3>
          ...
          <div class="bg-white rounded-2xl p-4 lg:p-5 flex flex-col items-center text-center shadow-[0_4px_20px_rgba(0,0,0,0.04)] border border-gray-100/50 hover:-translate-y-1 transition-transform group">
            ...
            <p class="text-[10px] lg:text-[11px] text-ink-soft leading-relaxed font-medium">Rule-based personalization.<br/> Automatically recommends review or next level based on quiz score.</p>
          </div>
          <!-- Card 4 -->
          <div class="bg-white rounded-2xl p-4 lg:p-5 flex flex-col items-center text-center shadow-[0_4px_20px_rgba(0,0,0,0.04)] border border-gray-100/50 hover:-translate-y-1 transition-transform group">
            <div class="text-3xl lg:text-4xl mb-3 h-12 flex items-center justify-center group-hover:scale-110 transition-transform">👩‍🏫📊</div>
        ...
        <!-- Big Actions -->
        <div class="flex flex-col sm:flex-row items-center justify-center gap-4 lg:gap-6 pt-10 pb-4">
          <RouterLink to="/register" class="w-full sm:w-auto bg-[#00E676] hover:bg-[#00c853] text-white px-16 py-3.5 rounded-full font-bold text-base shadow-xl shadow-green-500/20 transition-all transform hover:scale-105 border-0 text-center">
            Register
          </RouterLink>
          <RouterLink to="/login" class="w-full sm:w-auto bg-white text-brand-blue border-2 border-brand-blue hover:bg-gray-50 px-16 py-3.5 rounded-full font-bold text-base shadow-xl shadow-blue-500/10 transition-all transform hover:scale-105 text-center">
            Login
          </RouterLink>
        </div>
